        ...
        if (top == null || top.length == 0) {
            throw new IOException(
                "Bundled assets missing: \"" + assetDir + "/\" is empty or absent in the APK. "
                    + "Build with Maven so game files are copied into assets (see JMugen.Render.libgdx.android/pom.xml "
                    + "stage-mugen-data-into-android-assets), or copy the repo data/ tree into src/main/assets/" + assetDir + "/."
            );
        }
